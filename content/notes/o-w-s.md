...
Masses on springs are Simple Harmonic Oscillator.
...
KE = ½mv2
PE = ½kx2 
ETOTAL = PE + KE = ½kx2 + ½mv2
When PE is maximum, KE is zero, v is zero.
...
Review: Standing waves are waves that reflect and overlap on each other, which results in the peak amplitudes remaining in the same places.  
...
This would have N nodes between the two end anti-nodes.
Important note: the term resonance is synonymous what standing sound waves.
